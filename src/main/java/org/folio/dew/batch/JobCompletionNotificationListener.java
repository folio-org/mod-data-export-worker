...
public class JobCompletionNotificationListener extends JobExecutionListenerSupport {
    ...
    if (after) {
      ...
      if (jobExecution.getJobInstance().getJobName().contains(BULK_EDIT_UPDATE.getValue())) {
        ...
        if (localFilesStorage.notExists(filePath) && remoteFilesStorage.containsFile(filePath)) {
          int totalUsers = CsvHelper.readRecordsFromMinio(remoteFilesStorage, filePath, UserFormat.class, true).size();
  ...
  private void processJobAfter(String jobId, JobParameters jobParameters) {
    ...
    var names = localFilesStorage.walk(path)
      .filter(name -> FilenameUtils.getName(name).startsWith(fileNameStart))
      .collect(Collectors.toList());

    if (names.isEmpty()) {
      return;
    }
    names.forEach(localFilesStorage::delete);

    log.info("Deleted temp files {} of job {}.", names, jobId);
  }
  ...
  private boolean noRecordsFound(String pathToFoundRecords) throws Exception {
    if (localFilesStorage.notExists(pathToFoundRecords) && !remoteFilesStorage.containsFile(pathToFoundRecords)) {
      log.error("Path to found records does not exist: {}", pathToFoundRecords);
      return true;
    }
    return localFilesStorage.notExists(pathToFoundRecords) ?
      CsvHelper.readRecordsFromMinio(remoteFilesStorage, pathToFoundRecords, UserFormat.class, true).isEmpty() :
      localFilesStorage.lines(pathToFoundRecords).count() <= 1;
  }
  ...
  private String prepareChangedUsersFile(String path, String jobId) {
    ...
    try {
      var updatedUserFormats = CsvHelper.readRecordsFromFile(localFilesStorage, path, UserFormat.class, true)
        .stream()
        .filter(userFormat -> updatedIds.contains(userFormat.getId()))
        .collect(Collectors.toList());
      CsvHelper.saveRecordsToLocalFilesStorage(localFilesStorage, updatedUserFormats, UserFormat.class, path);
    } catch (Exception e) {
