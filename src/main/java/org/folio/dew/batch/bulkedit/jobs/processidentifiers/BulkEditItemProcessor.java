...
@Log4j2
public class String implements ItemProcessor<List<String>, String> {
  ...
  @Override
  public String process(List<String> uuids) {
    uuids.forEach(uuid -> {
      User user = userClient.getUserByQuery("id==" + uuid, 1);
      
    });
    final String servicePointName = servicePointMap.get(uuid.getServicePointId());
    var logExportFormat = new CirculationLogExportFormat();
    logExportFormat.setUserBarcode(uuid.getUserBarcode());
    logExportFormat.setDescription(uuid.getDescription());
    logExportFormat.setAction(uuid.getAction().getValue());
    logExportFormat.setDate(format.format(uuid.getDate()));
    logExportFormat.setServicePointId(servicePointName);
    logExportFormat.setObjectField(uuid.getObject().getValue());
    logExportFormat.setSource(uuid.getSource());

    final String items = uuid.getItems().stream()
