...
public class BursarFilterAggregateEvaluator {
  ...
  public static boolean evaluateAggregate(AggregatedAccountsByUser aggregatedAccounts, BursarExportFilterAggregate filter) {
    if (filter == null) {
      return true;
    }

    int numRows = aggregatedAccounts.getAccounts()
      .size();
    if (filter.getProperty() == PropertyEnum.NUM_ROWS) {
      ...
      return compareHelper(filter.getCondition(), aggregatedAccounts.findTotalAmount()
        .multiply(new BigDecimal("100"))
        .intValue(), filter.getAmount());
    } else {
  ...
  public static boolean evaluate(AggregatedAccountsByUser aggregatedAccounts, @NonNull BursarExportFilter filter) {
    if (filter instanceof BursarExportFilterPass) {
      return true;
    } else if (filter instanceof BursarExportFilterPatronGroup filterPatronGroup) {
      return (UUID.fromString(aggregatedAccounts.getUser()
        .getPatronGroup())
  ...
  public static final boolean compareHelper(ConditionEnum condition, int amount, int conditionAmount) {
    if (condition == null) {
      return true;
    }
