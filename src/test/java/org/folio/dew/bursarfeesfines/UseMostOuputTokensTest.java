...
class UseMostOuputTokensTest extends BaseBatchTest {
  ...
  void testUseMostOutputTokens() throws Exception {
    ...
    wireMockServer.verify(postRequestedFor(urlPathMatching(BursarFeesFinesTestUtils.TRANSFER_ACCOUNTS_ENDPOINT)));
    ...
    String expected = FileUtils.readFileToString(new FileSystemResource(EXPECTED_CHARGE_OUTPUT).getFile(), "UTF-8")
      .replaceAll("\\{CURRENT_YEAR_HERE\\}", LocalDate.now()
        .getYear() + "");

    assertThat(FileUtils.readFileToString(actualChargeFeesFinesOutput.getFile(), "UTF-8"), is(expected));
  }
}
